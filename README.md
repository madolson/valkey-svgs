...
nodes, which is what makes the particle counts here possible at all.
...
| `cluster-gossip` | Seven shards on a tilted ring trading messages across a full mesh, each arrival flashing the shard it reaches | Cluster mode, gossip, replication, membership |
