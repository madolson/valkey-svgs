...
Five animated banners, generated by [`motion.mjs`](motion.mjs). They share the static set's
...
| `blackhole-particles` | The accretion disk as thousands of orbiting points of light, far side lensed over the shadow, near side crossing in front | Talks, keynotes, anything that wants one striking abstract image |
| `cluster-gossip` | Six shards at the corners of a hexagon trading messages across a mesh, each arrival flashing the shard it reaches | Cluster mode, gossip, replication, membership |
| `performance-traffic` | Command traffic warping inward to a vanishing point at the mark, curving and stretching as it accelerates | Throughput, latency, benchmark posts |
| `eclipse-corona` | A black disc over the sun, the corona streaming outward in long equatorial lobes and short polar plumes, prominences looping off the limb | Talks, keynotes, launches, anything wanting one striking abstract image |
...
same set in the same places. `client-streams` and `performance-traffic` are both this.
